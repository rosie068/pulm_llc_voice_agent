...
| **New appointment** | Front desk (Mark) hands to scheduler (Linda) instantly — no permission question. Linda: spell back full name letter-by-letter; detect **new vs returning** (returning → "Welcome back", default follow-up; no record → confirm "are you a new patient?" before creating one, then default new-patient visit). Run §3.4 checklist (enforced in code by book_appointment); collect missing demographics ONE at a time (email → phone → address → insurance), **each confirmed by readback like the name** — email spelled letter-by-letter before the @ plus the domain in words, phone read back in digit groups, address read back in full with street number and ZIP digit-by-digit — saving each immediately; book per §5; **assign provider + location**; read prep script in full | Missing info caller can't supply; insurance issue agent can't clear |
...
| **Non-emergency symptom / asks for doctor** | Acknowledge without medical advice; ask one emergency-symptom safety question if urgency is unclear; then move straight to booking in the SAME turn: front desk says one send-off line ("I'm sorry you're dealing with that. Let me get you over to Linda, our scheduler — she can book you an appointment with one of our providers. One moment.") AND triggers the scheduler handoff in that turn — never the line without the handoff (dead air). Scheduler/monolith offers directly ("I can book you an appointment with one of our providers."), books on yes (§3.4 unchanged). Caller wants to talk to someone / asks a medical question at any point → clinical staff transfer. Neither works → capture full intake and create a clinical callback flag | Emergency symptoms → 911/ER + emergency page |
...
- **Clinical or symptom question** → the agent does not diagnose, interpret symptoms, recommend treatment, or suggest medication. It remains the caller's access point: acknowledge the concern and, once emergency warning signs are ruled out, **offer to book an appointment first** (offering the practice's own appointment is service, not medical advice — the agent still makes no urgency or disposition judgment; the caller chooses). Caller preferring to speak with someone, or asking an actual medical question → connect to the next available clinical staff member; if live transfer is unavailable, capture name, DOB, callback number, and concern one field at a time and create a clinical follow-up flag.
