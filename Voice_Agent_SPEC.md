...
| **Non-emergency symptom / asks for doctor** | Acknowledge without medical advice; ask one emergency-symptom safety question if urgency is unclear; then **offer an appointment first** in one short human sentence ("I'm sorry that cough's been lingering. I can book you an appointment with one of our providers."). Yes → normal scheduling flow (§3.4 unchanged). Caller instead wants to talk to someone / asks a medical question / hesitates → clinical staff transfer. Neither works → capture full intake and create a clinical callback flag | Emergency symptoms → 911/ER + emergency page |
